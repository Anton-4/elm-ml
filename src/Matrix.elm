...
import Helper exposing (arrToString)
import Array exposing (Array)
import Array.Extra
import Array exposing (Array)
...
type alias Column = Array Float

checkListAll : (Int -> Bool) -> List Int -> Bool
checkListAll f lst =
    case lst of
        x::xs -> if not <| f x then 
                    False 
                else
                    checkListAll f xs
        [] -> True
...
fromArray array =
    let
        nrRows = Array.length array
        nrColsPerArr = Array.map Array.length array
        firstArrLength = Array.get 0 nrColsPerArr
        rowLengths = Array.map (\row -> Array.length row) array
    in
    ...
    if n > 0 then
        let
            ...
            dataList = List.map
                (\rowNr ->
                    let
                        zerosBefore = List.repeat rowNr 0
                        zerosAfter = List.repeat (totalRows - rowNr) 0
                        row = zerosBefore ++ 1 :: zerosAfter
...
getCol mat colNr =
    ...
    Just arr -> Ok arr
    Nothing -> Err <| "colNr " ++ String.fromInt colNr ++ " was out of bounds for matrix with " ++ String.fromInt mat.nrCols ++ " cols."

transformCol : Matrix -> Int -> (Column -> Result String Column) -> Result String Matrix
transformCol mat colNr transFun =
    let
        colRes = getCol mat colNr
...
updateCol mat colNr newCol =
    let
        updateMatrix = \newMatCol -> 
            Ok { data= Array.set colNr newMatCol mat.data
            , nrRows= mat.nrRows
            , nrCols= mat.nrCols
            }
        colLength = Array.length newCol
    in
        if mat.nrRows == colLength then
            updateMatrix newCol
        else
            Err <| "New Column does not have same length (" ++
                String.fromInt colLength ++ ") as number of matrix rows (" ++
            ...
            let
                ...
                zip2D list2D =
                    case list2D of
                        (x::xs)::xss -> 
                            let
                                otherHeads =  Maybe.Extra.values <| List.map List.head xss
                                otherTails = Maybe.Extra.values <| List.map List.tail xss
                            in
                                (x :: otherHeads) :: zip2D (xs :: otherTails)
                        [] :: xss -> zip2D xss
                        [] -> [] 

                dataLists = Array.toList <| Array.map Array.toList matrix.data
                transposedList = zip2D dataLists
                transposedArr = Array.fromList <| List.map Array.fromList transposedList
            in
    ...
    let
        mulArray =
            Array.map
                (\row ->
                    let
                        transpB = transpose matB
                        products = Array.map (\col -> dotProduct row col) transpB.data 
                    in 
                        products)
                matA.data
    in
